...
function getRandomWaitTime() {
  ...
  const weights = [0.4, 0.35, 0.20, 0.05];
...
// Simulate a reporter standing within ~200m of the center
function fakeUserLocation(centerLat, centerLng) {
  const offset = () => (Math.random() - 0.5) * 0.004; // ~200m
  return { lat: centerLat + offset(), lng: centerLng + offset() };
}

async function seedDatabase() {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/niko-kadi');
    ...
    const centersWithQueue = ALL_IEBC_CENTERS.map(center => ({
      ...
      address: center.location || center.landmark,
    ...
    for (let i = 0; i < 500; i++) {
      ...
      const reportTime = new Date(twoHoursAgo.getTime() + Math.random() * 2 * 60 * 60 * 1000);
      const userLoc = fakeUserLocation(center.coordinates.lat, center.coordinates.lng);
